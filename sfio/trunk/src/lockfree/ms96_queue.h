...
*/
LIBLFDS_CALLING_CONVENTION(void) ms96_queue_initialize( ms96_queue_t* queue, ms96_queue_node_t* node );

/* deinitialize queue, expects queue to be empty and to stay that way
    return the dummy link node used by the queue
*/
LIBLFDS_CALLING_CONVENTION(void) ms96_queue_terminate( ms96_queue_t* queue, ms96_queue_node_t** node );


/* to enqueue, supply a node (allocated from somewhere), and a value
...
*/
LIBLFDS_CALLING_CONVENTION(void) ms96_queue_enqueue( ms96_queue_t* queue, ms96_queue_node_t* node, ms96_value_t value );

/* dequeue returns a node, and the value
    returns non-zero on success, zero if the queue was empty
*/
LIBLFDS_CALLING_CONVENTION(int) ms96_queue_dequeue( ms96_queue_t* queue, ms96_queue_node_t** node, ms96_value_t* value );
